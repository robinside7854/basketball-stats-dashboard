...
- 날짜를 열면 `POST /games {date}` 가 `leagues.games_per_round` 만큼 슬롯을 만든다(빠진 번호만 채운다).
  **친선 슬롯이 하나라도 있으면 이 일괄 생성은 돌지 않는다.**
  ⚠ **미라클은 2026-09-07 에 9 → 3 으로 바꿨다.** 하루 9경기 로테이션에서 **3대진 × 3~4쿼터**로
  진행 방식이 바뀌었기 때문이다(승패는 경기 단위 = 쿼터 합산 점수). 그 이전 날짜의 9칸은 그대로다.
...
- **칸이 모자라면 몇 칸인지 알려준다** — 응답 `need_slots`. 9/19 처럼 영상이 9개인데
  `games_per_round` 가 3이면 4~9번이 통째로 빠진다(9/7 에 9→3 으로 바꿨다).
