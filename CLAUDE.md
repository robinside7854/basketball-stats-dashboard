...
- 날짜를 열면 `POST /games {date}` 가 `leagues.games_per_round` 만큼 슬롯을 만든다(빠진 번호만 채운다).
  **친선 슬롯이 하나라도 있으면 이 일괄 생성은 돌지 않는다.**
  ⚠ **미라클은 9 → 3 → 다시 9 로 오갔다.** 9/7 에 3대진 × 3~4쿼터로 바꿔 3 으로 내렸다가,
  **9/19 부터 하루 9경기로 되돌렸다(사용자 확인 2026-09-19, "앞으로 계속 9경기")** → 설정값 9.
  값은 리그 `/settings` → 「정규일정 당 경기 수」(입력 상한 10)에서 바꾼다. 진행 방식이 또
  바뀌면 여기부터 고친다 — **짐작하지 말고 운영진에게 확인한다.**
  ⚠ 이 값을 올리면 **지나간 날짜도 다시 열 때 늘어난다.** 일괄 생성은 "빠진 번호를 채우는"
  방식이라, 3칸으로 만든 9/12 를 기록 화면에서 열면 4~9번 빈 슬롯이 그 자리에서 생긴다
  (기록이 없으니 집계에는 영향이 없고, 거슬리면 「슬롯 삭제」로 지운다 —
  이벤트가 하나라도 있으면 서버가 409 로 막는다).
...
### 기록 시작 전 화면 = 「기록 준비」 카드 (2026-09-07 재구성)

슬롯을 고르면 **정할 것만** 보인다: ①경기 종류 `[정규전][친선전]` ②팀 ③영상(접힘) ④고급(접힘).

- **경기 종류가 분기점이다.** 종전엔 「친선전으로 표시」 버튼이 다른 버튼 다섯 개 사이에 섞여 있었다 —
  잘못 눌리면 그 경기가 순위·개인 스탯에서 통째로 빠지는데 지금 상태는 배지 하나로만 알 수 있었다.
  대회 묶음에서는 고를 것이 없으므로(정의상 공식전) 버튼 대신 `대회 경기` 배지만 둔다.
- **접은 것은 없앤 것이 아니다.** 영상 패널·+1 설정은 `<details>` 안에 그대로 있다.
  영상이 아직 없는 슬롯은 자동으로 펼쳐지고, +1 이 지정돼 있으면 접힌 채로도 배지가 보인다.
  ⚠ `videoPanelOpen` 을 되살리는 effect 의 의존성에 **`quarterVideos` 를 넣지 말 것** —
  1쿼터를 붙이는 순간 패널이 스스로 닫혀 2~4쿼터를 붙이던 손이 끊긴다.
- 근거는 실측이다(리그 282경기): 영상 100% 자동 연결 · 경기한정 +1 4% · **쿼터별 +1 0%**.
  덜어낼 대상을 고를 때는 짐작이 아니라 이 숫자로 판단한다.
- ⚠ **「대진 자동 채우기」는 2026-09-07 에 뺐다.** 「승자 잔류 · 2연속 휴식」은 하루 9경기
  로테이션 규칙이라 3대진 방식에서는 **틀린 대진을 채운다.** 규칙 자체는
  `src/lib/league/rotation.ts` 에 남겨 뒀다 — 되살리려면 버튼과 핸들러만 다시 붙이면 된다.
...
`isPlusOneFor(playerId, game, globalSet, quarter)` — **quarter 는 필수 인자다.**
...
4. 위에서 +1 이 된 뒤, `league_games.plus_one_quarters`(113)에 그 선수 지정이 있으면
   **그 쿼터에서만** +1 이다. `{ "<playerId>": [1,2] }` — 키가 없으면 전 쿼터(기존 동작).

- ⚠ **`quarter` 를 옵셔널로 바꾸지 말 것.** 안 넘긴 곳이 조용히 "전 쿼터 +1" 로 계산돼
  화면마다 점수가 갈린다 — 이 함수가 없애려던 바로 그 병이다. 필수로 두면 **컴파일러가
  호출부 26곳을 전부 열거해 준다.** 쿼터를 알 수 없는 자리는 `null` 을 넘겨 그 사실을 드러낸다.
- ⚠ 새 집계는 이벤트 select 에 **`quarter`**, 게임 select 에 **`plus_one_quarters`** 를 함께 읽어야 한다.
  하나라도 빠지면 그 화면만 옛 판정으로 돌아간다.
- `scripts/verify-scoring.mjs` 의 SQL 은 이 판정의 **복제본**이다 — 규칙을 바꾸면 거기도 같이 고친다.
...
- **자동 매핑** `syncYoutubeForLeague.ts` — 제목 규칙이 **두 가지**다(2026-09-07).
  수동 실행(`POST /youtube-sync`) · 주간 cron · 경기 시작 시 백그라운드 훅이 모두 이 함수를 쓴다.
  | 제목 | 해석 | 저장 위치 |
  |---|---|---|
  | `260905 지피티vs빅현욱 1Q` | 그날 그 **대진의 1쿼터** | `league_game_videos` + 대표 자동 갱신 |
  | `260919 경기1` · `260801 경기 3` | 그날 **N번 슬롯** | `league_games.youtube_url` |
  ⚠ **번호형은 옛 형식이 아니라 현역이다.** 업로더의 습관이 날마다 다르다 — 9/5 는 쿼터형,
  **9/19 는 다시 번호형**(`260919 경기1`~`경기9`)이었다. 어느 쪽도 지우지 말 것.
  그날 영상에 쿼터형이 **하나라도** 있으면 쿼터형으로만 처리한다 — 섞으면 같은 영상이 대표와
  쿼터에 이중으로 붙어 화면마다 다른 것이 재생된다. 제목 파싱은 `videoTitle.ts` 가 전담한다
  (제목을 못 읽은 것과 팀을 못 찾은 것은 운영자가 할 조치가 다르므로 함수를 나눠 뒀다).
- **규칙을 손으로 고를 수 있다 (2026-09-19)** — `POST /youtube-sync { titleRule }`,
  `'auto'`(기본·그날 영상을 보고 고름) / `'legacy'`(번호형 강제) / `'quarter'`(쿼터형 강제).
  자동 판정은 다수결이 아니라 "쿼터형이 하나라도 있는가" 라, 제목이 섞여 올라온 날에는
  쿼터형 1개가 번호형 8개를 이긴다. 기록 화면은 응답의 `rule_counts` 를 보고 토스트에
  「'경기 번호' 규칙으로 다시 연동」 버튼을 띄운다 — **규칙이 둘이라는 사실을 운영자가
  화면에서 알 수 있어야** "왜 안 붙었지"에서 멈추지 않는다.
- **칸이 모자라면 몇 칸인지 알려준다** — 응답 `need_slots`. 영상 수보다 그날 칸이 적으면
  나머지가 통째로 빠진다(9/19 에 실제로 겪었다 — 영상 9개 · 칸 3개).
  설정값을 9로 되돌린 뒤에는 평소에 0 이지만, 하루에 10경기가 나온 날에는 다시 쓰인다.
  ⚠ 연동은 여전히 **슬롯을 만들지 않는다**(2026-08-22 사고). 만드는 것은 사람이 누르는
  「+ 추가」(`POST /games { addSlot:true }`, 그 날짜의 친선 여부를 상속)뿐이고, 토스트의
  「칸 N개 늘리고 다시 연동」 버튼이 그 경로를 N번 호출한 뒤 재연동한다.
- **회귀 검증**: `node scripts/verify-video-title.mjs` — 두 규칙이 서로를 침범하지 않는지,
  NFD 제목·두 자리 경기 번호가 읽히는지 대조한다(DB·네트워크 없음). 규칙을 바꾸면 같이 고친다.
- **팀 판정은 `league_team_aliases` 에 등록된 표기만 인정한다** (마이그레이션 114, 정본
  `teamNameIndex.ts`). 업로더는 팀명을 줄여 쓴다 — 실제 팀명 `챗지피지기` 인데 제목은 `지피티` 다.
  ⚠ **유사도로 추측해서 붙이지 않는다.** 유사도는 오직 "혹시 챗지피지기?" 안내 문구에만 쓴다.
  이름의 출처는 셋: 기본 팀명 · **그 분기의 팀명**(override — 미라클은 분기마다 팀명이 바뀐다) · 별칭.
  등록은 리그 `/settings` → **영상 제목의 팀 별칭**.
- **자동 팀 매핑** — 제목이 대진을 알려주므로, 그 대진의 경기가 아직 없으면 **팀이 비어 있고
  기록 전인 슬롯**을 하나 가져다 home/away 를 채운다(업로드가 이른 대진부터). ⚠ 팀이 이미 있거나
  `is_started` 인 경기는 절대 건드리지 않는다 — 팀만 바꾸면 `league_game_events.team_id` 가
  무관한 팀을 가리켜 그 선수들이 박스스코어에서 사라진다(그 이관은 `reassign-teams` 가 한다).
- **같은 대진이 그날 두 칸 이상이면 붙이지 않는다.** 승자 잔류 로테이션 시절엔 흔한 일이었다.
  추측하면 조용히 틀리므로 "목록에서 고르기"로 넘긴다.
- **검색어는 날짜(yymmdd) 하나뿐.** `경기` 같은 단어를 붙이면 제목 규칙이 바뀐 날 영상이 통째로
  빠진다 — 9/5 영상 10개가 하나도 안 걸린 원인 중 하나다. 목록 라우트와 같은 검색어를 써야
  두 경로가 같은 영상을 본다.
- **`dryRun: true`** (`POST /youtube-sync`) — 저장 없이 "무엇을 하려 했는지"만 돌려준다.
  제목 규칙이 또 바뀌면 **운영 데이터로 먼저 이걸 돌려 보고** 손댄다.
...
- ~~리그는 한 슬롯 = 영상 1개~~ **폐기(2026-09-07)** — 미라클이 쿼터제로 바뀌어 리그도 영상이
  대진×쿼터로 올라온다. 기록 화면의 쿼터 4칸은 이제 리그에서도 보인다(종전엔 대회 전용).
  쿼터 영상이 0건인 옛 경기는 대표 폴백으로 종전과 100% 같게 동작한다.

### 쿼터별 영상 (마이그레이션 111) — 대회 · 리그 공통
...
- ⚠ `round_label` 은 **`조별예선·32강·16강·8강·4강·준결승·결승` 만** 받는다. 대회 보드의 성적 판정이
  아는 값이라야 우승·N강 탈락이 표시된다.
  - **정본은 `src/lib/tournament/rounds.ts` 하나뿐이다** (2026-09-19). 목록(`ROUND_LABELS`) ·
    깊이(`ROUND_DEPTH`) · 정렬 역순(`ROUND_PRIORITY`) · 성적 가중치(`ROUND_POINTS`·`roundPoints()`) ·
    토너먼트 라운드(`KNOCKOUT_ROUNDS`)가 전부 `ROUND_TABLE` 한 벌에서 파생된다.
    ⚠ **새 라운드는 그 표에만 추가한다.** 종전엔 같은 표가 9벌로 흩어져 값 체계까지 서로 달랐고,
      실제로 GameForm 에는 `준결승` 이, 박스스코어 정렬 맵에도 `준결승` 이 빠져 있었다
      (`isPlusOneFor()` · `resolveGameVideo()` 와 같은 병).
- ⚠ **대회 이름 비교도 정본 하나다 — `src/lib/tournament/name.ts`.** 「경기 자동 연동」의 기존 대회
  매칭과 `POST /api/tournaments` 의 중복 방어(findOrCreate)가 같은 `normalizeTournamentName()` 을
  쓴다. 두 벌로 갈라지면 한쪽은 같은 대회로 보고 한쪽은 새로 만들어, 운영 DB 에 「2026 바다배」가
  두 개 생긴 그 상태로 돌아간다. **연도가 다르면 부분 포함만으로 묶지 않는다.**
...
### ⚠ 대회는 "우리 한 팀" 이다 — 기록기에서 리그 개념 3종이 빠진다

리그는 양 팀 모두 우리 동호회 사람이라 홈·어웨이를 나란히 기록한다. 대회는 상대가 외부
동호회다 — 명단조차 알 수 없고, 그 기록이 우리 통계에 섞이면 안 된다.

- **감춘 것**: 어웨이 드롭존(우리 팀 한 칸만) · 선발 카드의 `→ 상대팀` 이동 · 타팀 임시 출전 ·
  비정규 선수 추가 · 미배정 풀 · 입력 패드의 「상대 등록」(`opponentRecording={false}`).
- **상대 점수는 마감할 때 최종 점수 하나로 넣는다**(112, 사용자 결정 2026-08-30) —
  `league_games.opponent_score_manual`. 기록 화면의 마감 모달에 입력칸이 있고,
  마감된 경기는 「다시 기록하기」 → 마감으로 고친다(저장된 값이 채워진다).
  ⚠ **`away_score`/`home_score` 에 직접 넣지 말 것** — `recompute` 가 이벤트에서 재계산해
  덮어쓰므로 다음 재계산에 조용히 사라진다. 재계산이 이 컬럼을 **외부 팀 쪽에만** 반영한다.
  - 실시간 `[+1][+2][+3]` 방식은 **뺐다.** 그 경로로 만들던 `opp_score_1|2|3` 이벤트 타입과
    `scoring.ts` 의 `OPPONENT_SCORING` 은 남겨 뒀다(재계산이 폴백으로 합산한다) —
    되살리려면 버튼만 다시 붙이면 된다.
- ⚠ **선수 없는 이벤트를 거르는 코드를 조심할 것.** `recompute` 가 그랬고, 그 상태로는
  **상대 점수가 항상 0**이다(화면은 멀쩡하고 한쪽 스코어만 빈다). `events` POST 도
  `.eq('id', null)` 로 500 이 났다 — 선수가 없으면 플러스원 조회를 건너뛴다.
- ⚠ **대회에서는 선발을 자동 체크하지 않는다.** 리그는 팀당 5명 안팎이라 "정규 전원 체크"가
  곧 코트 5명이었지만, 대회 참가 명단은 10명이 넘는 게 정상이라 그 기본값이 켜지면
  `팀당 선발 최대 5명` 에 걸려 **경기 시작 버튼이 아예 안 먹는다**.
...
- ⚠⚠ **`league_players` 를 `league_id` 로 찾지 말 것 — 대회 묶음에서 0명이 나온다.**
  선수는 팀 소유다(087). 실측: 대회에서 `league_id` 로 0명 / `team_id` 로 46명.
  조회가 비면 **에러 없이** 이름이 `알 수 없음` 으로, `plus_one` 은 전부 꺼진 것으로 계산된다 —
  숫자가 그럴듯해서 표만 봐서는 원인이 안 드러난다(2026-08-31 에 스탯 화면 전체가 이 상태였다).
  반드시 `.eq('team_id', await resolveTeamId(leagueId))`.
  리그에서는 두 조건의 결과가 같으므로(46 = 46) 바꿔도 기존 화면은 안 바뀐다.
