...
1. `league_games.plus_one_player_id` 가 있으면 **그 선수 한 명만** 그 경기의 +1 (충돌 해소용 단일 지정)
2. 없으면 `league_players.plus_one` 플래그 (전역)
- 가산점은 `leagues.rules.plus_one_bonus`(미라클 = 야투에 +1). ⚠ **선수 플래그를 켜면 과거 마감
  경기까지 소급된다** — 그 선수의 과거 야투마다 점수가 올라가 순위·기록이 바뀐다.
  "이번 경기만 +1 한 명 추가" 는 현재 구조에 **없다**(단일 지정은 추가가 아니라 배타 지정).
...
- **한 슬롯 = 영상 1개.** 쿼터별로 쪼갠 영상은 대표 하나만 저장되고, 기록 중에는 입력칸으로 갈아끼운다.
