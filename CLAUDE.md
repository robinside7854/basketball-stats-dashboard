...
- **상대 점수**는 기록 중 `[+1][+2][+3]` 로만 남긴다 — 선수 없이 **팀에만** 붙는 이벤트
  (`league_player_id=null`, `team_id=상대팀`). **타입이 곧 점수**다: `opp_score_1|2|3`
  (`scoring.ts` 의 `OPPONENT_SCORING`, `fetchScoringRules` 가 시즌 룰 밑에 깐다).
  ⚠ `opp_score` 하나에 가변 points 를 담지 말 것 — events POST 는 클라이언트 points 를
  무시하도록 설계돼 있고, 그걸 믿기 시작하면 저장값과 재계산값이 갈린다.
