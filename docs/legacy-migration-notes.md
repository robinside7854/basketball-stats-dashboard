...
## 단계 C-5 — 화면 전환 실행 (파란날개 → 리그 주소)

2026-08-05, `src/middleware.ts` 에 실제 리다이렉트를 걸어 사용자가 보는 화면을 바꿨다.
이 마이그레이션 전체에서 **사용자가 보는 화면이 바뀌는 유일한 커밋**이다.
...
하위 경로(`/boxscore`·`/gamelog`·`/tournaments` 등)는 그대로 이어붙이지 않는다. 레거시
트리의 이 이름들은 리그 트리에 같은 이름이 없거나(`/gamelog`→리그엔 없음) 뜻이 다르다
(`/boxscore`는 리그에서 날짜별 라우트 `boxscore/[date]`라 시즌 전체를 뜻하는 레거시와
모양이 다르다). 이어붙이면 404 가 나므로, 하위 경로가 있으면 무조건 리그 홈으로 보낸다.

### 왜 301 이 아니라 307 인가

301(영구 이동)은 브라우저가 무기한 캐시한다. 만약 새 화면에 문제가 생겨 이 리다이렉트
블록을 지워도, 이미 301 을 캐시한 사용자의 브라우저는 서버에 다시 묻지 않고 계속 새
주소로 간다 — 롤백을 해도 사용자에게 닿지 않는다. 307(임시 이동)은 캐시되지 않으므로,
블록을 지우면 다음 요청부터 즉시 레거시로 돌아간다. 전환이 며칠~몇 주 안정적으로
운영된 뒤 단계 D 에서 307 을 301 로 승격한다(그때는 롤백 가능성보다 캐싱 이득이 크다고
판단한 시점).

### `/league/paranalgae` 모호성 — 선택 화면으로 해결

`src/app/league/[orgSlug]/page.tsx` 를 org 안에 활성/예정 리그가 여러 개면 자동으로
하나를 고르지 않고 청년부·장년부 두 카드를 보여주는 선택 화면으로 바꿨다(둘은 서로 다른
팀이라 자동 선택하면 방문자 절반이 틀린 화면으로 감). `pana-basket-senior` 빈 스텁
리그(`leagues.id=76f91f2f-…`)는 실측 결과 **`org_slug` 자체가 `pana-basket-senior`로
`paranalgae`와 다르다** — 그래서 `/league/paranalgae` 쿼리(`.eq('org_slug', orgSlug)`)에
애초에 걸리지 않고, 이 화면에도 나타나지 않는다. 스텁을 지우거나 정리하는 결정은
여전히 필요 없다 — 단계 D 로 그대로 넘긴다(하위 데이터 0건이라 방치해도 무해).

선택 화면은 org 스코프(`org_slug` 필터)로만 다른 동호회의 존재를 가린다 — 이건
기존 쿼리 조건 그대로다. 추가로, 같은 org 안에서도 리그별 공개/비공개
(`teams.is_public`, `isLeaguePublic()`)를 따로 확인해 비공개 리그는 그 리그에 이미
승인된 세션이 있는 방문자에게만 보여준다. 실측: 파란날개 청년부·장년부 팀 둘 다
`is_public=true`라 현재는 두 카드 모두 노출된다.

### 실측 검증 (dev 서버, 2026-08-05)

리다이렉트 매트릭스(하위 경로 포함) 8개 전부 307 + 정확한 `Location` 확인.
`/league/paranalgae` 는 200(선택 화면, "청년부"·"장년부" 텍스트 확인, "미라클"·
"pana-basket-senior"·"자체전" 등 다른 동호회/스텁 관련 문자열 없음 확인).
`/league/miracle/2026` 200 확인 — 미라클 무영향.

**롤백 실측**: `git checkout -- src/middleware.ts` 로 이 커밋 이전 버전으로 되돌린 뒤
서버를 재기동해 `/paranalgae/youth` 를 호출하면 리다이렉트 없이 200 으로 레거시 화면
(청년부 레거시 페이지)이 그대로 렌더됨을 확인했다 — 레거시 트리는 전혀 손대지
않았으므로 미들웨어 블록만 지우면 그 즉시 레거시 경험으로 복귀한다. 확인 후 다시
새 미들웨어로 복원해 리다이렉트 8개가 재기동 후에도 그대로 통과함을 재확인했다.
