...
- **9/19 는 9경기다 (사용자 확인).** `경기1~9` = 슬롯 1~9 로 1:1. 쿼터로 쪼갠 날이 아니므로
  대진 롤업은 신경 쓸 것이 없다.
  ⚠ 다만 `games_per_round` 는 9/7 에 **3으로 바꾼 그대로**다 — 날짜를 열면 3칸만 생긴다.
  9경기 날은 기록 화면에서 칸을 9개로 늘려야 한다(「+ 추가」 6번, 또는 연동 토스트의
  「칸 6개 늘리고 다시 연동」 한 번). 진행 방식이 9경기로 **굳으면** 설정값을 9로 되돌리는 게
  맞지만, 그러면 **모든 날짜가 같이 늘어나므로** 사용자와 먼저 상의한다.
