...
- ⚠ **확인 필요**: 9/19 의 `경기1~9` 가 **9경기**인지 **3대진 × 3쿼터**인지는 제목만으로는
  알 수 없다. 번호형은 슬롯 1:1 이므로 후자라면 슬롯을 쿼터로 쓰는 셈이 된다 —
  `daily-boxscore` 의 대진 롤업(같은 대진 + slot_num 연속)이 이를 한 경기로 묶어 주므로
  화면상 전적은 맞지만, 팀을 채워 넣어야 롤업이 돈다.
