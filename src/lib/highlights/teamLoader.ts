...
// 대회 클러치 정의 (2026-07-18 개편) — 쿼터에 따라 판정이 다르다:
//
//   · 4쿼터  : 4쿼터 종료 300초 이내 AND 최종 점수차 2포제션(6점) 이내
//   · 연장    : 조건 없이 전부 클러치
//              (정규시간 동점으로 끝나야 연장이므로 그 자체가 접전 — 한 점 한 점이 결정적)
//
// 상대팀 선수 이벤트는 미기록이라 러닝 마진 산출 불가 → 최종 점수(games.our_score/opponent_score) 프록시로 근사
//
// 4쿼터 창이 120 → 300초로 늘어난 이유:
...
const TOURNAMENT_REGULATION_LAST_QUARTER = 4   // 5 이상 = 연장
...
// 게임별 4쿼터 마지막 성공슛 timestamp — 정규시간 종료 근사 기준점
// (실제 게임 클록은 미기록이라 마지막 득점 시각으로 근사)
// 연장 경기라도 4쿼터 슛은 "4쿼터 종료" 기준으로 재야 정규시간 막판 동점슛이 살아난다.
function buildGameQ4EndMap(events: TeamEvtRow[]): Record<string, number> {
  const map: Record<string, number> = {}
  for (const e of events) {
    if (e.quarter !== TOURNAMENT_REGULATION_LAST_QUARTER) continue
...
function isTournamentClutch(
  ...
  if (ev.quarter === null) return false
  // 연장 — 정규시간 동점 종료가 전제이므로 모든 득점이 클러치
  if (ev.quarter > TOURNAMENT_REGULATION_LAST_QUARTER) return true
  if (ev.quarter !== TOURNAMENT_REGULATION_LAST_QUARTER) return false
...
export async function loadTournamentHighlightDetail(
  ...
  // 3-b. 게임별 4쿼터 종료 timestamp + 최종 마진 사전계산 (클러치 판정)
