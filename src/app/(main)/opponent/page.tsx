...
import { Plus, Trash2, ChevronDown, ChevronRight, Undo2, Pencil, Check, X, Play } from 'lucide-react'
...
function ShotBarLabeled({
  ...
  if (total === 0) return <span className="text-[var(--mm-muted)] text-xs">-</span>
...
function DonutChart({ breakdown, size = 80 }: { breakdown: ShotBreakdownMap; size?: number }) {
  const total = SHOT_TYPES.reduce((s, t) => s + (breakdown[t.type]?.att ?? 0), 0)
  if (total === 0) return <div style={{ width: size, height: size }} className="rounded-full bg-[var(--mm-panel-alt)]" />
...
function TeamShotSummary({ breakdown, totalGames }: { breakdown: ShotBreakdownMap; totalGames: number }) {
  ...
  if (totalAtt === 0) return <p className="text-[var(--mm-muted)] text-sm">기록된 슈팅 없음</p>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mt-2">
        {sorted.map(({ type, label, color, b }) => {
          ...
          return (
            <div key={type} className="bg-[var(--mm-panel-alt)]/60 rounded-xl p-3 border border-[var(--mm-rule)]/40 flex flex-col gap-1">
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm shrink-0" style={{ backgroundColor: color }} />
                <span className="text-xs font-semibold text-[var(--mm-ink)]">{label}</span>
              </div>
              <div className="text-2xl font-black text-[var(--mm-ink)] mt-0.5">{pct}%</div>
              <div className="text-xs text-[var(--mm-muted)]">{b.made}/{b.att} 성공</div>
              <div className="text-xs text-[var(--mm-muted)]">성공률 {fgPct}%</div>
              <div className="text-xs text-[var(--mm-muted)]">{totalGames > 0 ? (b.att / totalGames).toFixed(1) : 0}회/경기</div>
            </div>
          )
        })}
      </div>
...
export default function OpponentPage() {
  ...
  const tabStyle = (t: Tab) => cn(
    ...
    tab === t ? 'border-[color:var(--mm-yellow)] text-[var(--mm-yellow-strong)]' : 'border-transparent text-[var(--mm-muted)] hover:text-[var(--mm-ink)] hover:border-[var(--mm-rule)]'
  )

  return (
    <div className="container mx-auto px-4 py-6 max-w-[1600px]">
      <h1 className="text-2xl font-bold mb-1">상대팀 분석</h1>
      <p className="text-[var(--mm-muted)] text-sm mb-6">상대팀 경기 기록 및 공격 패턴 분석</p>

      {/* Team selector bar */}
      <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl p-4 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-sm text-[var(--mm-muted)] shrink-0">팀 선택</span>
          <select
            className="bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)] min-w-[180px]"
          ...
          <div className="flex gap-2 ml-auto">
            <input value={newTeamName} onChange={e => setNewTeamName(e.target.value)}
              ...
              placeholder="새 팀 이름"
              className="bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)] w-36" />
            <button onClick={addTeam} className="flex items-center gap-1 px-3 py-1.5 bg-[var(--mm-ink)] hover:brightness-95 text-[var(--mm-panel)] text-sm rounded-lg transition-colors">
              <Plus size={14} /> 팀 추가
            </button>
          </div>
        </div>
      ...
      {!selectedTeam ? (
        <div className="text-center py-20 text-[var(--mm-muted)]">팀을 선택하거나 새로 추가하세요</div>
      ) : (
        <>
          {/* Tabs */}
          <div className="flex border-b border-[var(--mm-rule)] mb-6">
            ...
            <div className="ml-2 flex items-center">
              <button onClick={() => deleteTeam(selectedTeam.id)} aria-label="상대팀 삭제" className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] text-[var(--mm-muted)] hover:text-red-400 transition-colors">
                <Trash2 size={14} />
              </button>
            </div>
          </div>
          ...
          {tab === 'manage' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Players */}
              <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl p-4">
                <h2 className="font-semibold mb-3 text-[var(--mm-ink)]">선수 등록</h2>
                <div className="flex gap-2 mb-4">
                  <input value={newPlayerNumber} onChange={e => setNewPlayerNumber(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && addPlayer()}
                    placeholder="등번호 (예: 7)"
                    className="bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)] flex-1" />
                  <button onClick={addPlayer} className="px-3 py-1.5 bg-[var(--mm-ink)] hover:brightness-95 text-[var(--mm-panel)] text-sm rounded-lg transition-colors">
                    <Plus size={14} />
                  </button>
                </div>
                <div className="space-y-1.5 max-h-72 overflow-y-auto">
                  {players.map(p => (
                    <div key={p.id} className="flex items-center gap-2 bg-[var(--mm-panel-alt)] rounded-lg px-3 py-2">
                      <span className="font-bold text-[var(--mm-ink)] w-10 shrink-0">#{p.number}</span>
                      {editingPlayer === p.id ? (
                        <>
                          <input
                            value={editingName}
                            ...
                            autoFocus
                            placeholder="이름 입력"
                            className="flex-1 bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded px-2 py-0.5 text-sm text-[var(--mm-ink)]"
                          />
                          <button onClick={() => savePlayerName(p.id)} className="text-green-400 hover:text-green-300"><Check size={14} /></button>
                          <button onClick={() => setEditingPlayer(null)} aria-label="이름 편집 취소" className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] text-[var(--mm-muted)] hover:text-[var(--mm-ink)]"><X size={14} /></button>
                        </>
                      ) : (
                        <>
                          <span className="flex-1 text-sm text-[var(--mm-ink)]">{p.name ?? <span className="text-[var(--mm-muted)] italic">이름 없음</span>}</span>
                          <button onClick={() => { setEditingPlayer(p.id); setEditingName(p.name ?? '') }}
                            className="text-[var(--mm-muted)] hover:text-[var(--mm-ink)] transition-colors"><Pencil size={13} /></button>
                          <button onClick={() => deletePlayer(p.id)} aria-label="선수 삭제" className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] text-[var(--mm-muted)] hover:text-red-400 transition-colors"><Trash2 size={13} /></button>
                        </>
                      )}
                    </div>
                  ))}
                  {players.length === 0 && <p className="text-[var(--mm-muted)] text-sm text-center py-4">등록된 선수 없음</p>}
                </div>
              </div>

              {/* Games */}
              <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl p-4">
                <h2 className="font-semibold mb-3 text-[var(--mm-ink)]">경기 관리</h2>
                <div className="space-y-2 mb-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-[var(--mm-muted)] mb-1 block">날짜</label>
                      <input type="date" value={newGameDate} onChange={e => setNewGameDate(e.target.value)}
                        className="w-full bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)]" />
                    </div>
                    <div>
                      <label className="text-xs text-[var(--mm-muted)] mb-1 block">상대방 (vs)</label>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-[var(--mm-muted)] mb-1 block">대회명</label>
                      <input value={newGameTournament} onChange={e => setNewGameTournament(e.target.value)}
                        placeholder="선택"
                        className="w-full bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)]" />
                    </div>
                    <div>
                      <label className="text-xs text-[var(--mm-muted)] mb-1 block">라운드</label>
                  ...
                  <div>
                    <label className="text-xs text-[var(--mm-muted)] mb-1 block">YouTube URL</label>
                    <input value={newGameYouTubeUrl} onChange={e => setNewGameYouTubeUrl(e.target.value)}
                      placeholder="https://youtu.be/..."
                      className="w-full bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)]" />
                  </div>
                  <div>
                    <label className="text-xs text-[var(--mm-muted)] mb-1 block">영상 시작 오프셋 (분:초)</label>
                    <input
                  ...
                  <button onClick={addGame} className="w-full py-1.5 bg-[var(--mm-ink)] hover:brightness-95 text-[var(--mm-panel)] text-sm rounded-lg transition-colors">
                    경기 추가
                  </button>
                </div>
                <div className="space-y-2 max-h-52 overflow-y-auto">
                  {games.map(g => (
                    <div key={g.id} className="flex items-center justify-between bg-[var(--mm-panel-alt)] rounded-lg px-3 py-2 text-sm">
                      <div className="min-w-0 flex items-center gap-1.5 flex-wrap">
                        {g.is_complete && <Check size={13} className="text-green-400" aria-hidden />}
                        <span className={cn('font-medium', g.is_complete ? 'text-[var(--mm-muted)]' : 'text-[var(--mm-ink)]')}>{g.date}</span>
                        {g.vs_team && <span className="text-[var(--mm-ink)] text-xs">vs {g.vs_team}</span>}
                        {g.round && <span className="text-[var(--mm-muted)] text-xs">{g.round}</span>}
                        {g.youtube_url && <Play size={11} className="text-green-500 fill-current" aria-hidden />}
                      </div>
                      <button onClick={() => deleteGame(g.id)} aria-label="경기 삭제" className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] text-[var(--mm-muted)] hover:text-red-400 transition-colors ml-2 shrink-0">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ))}
                  {games.length === 0 && <p className="text-[var(--mm-muted)] text-sm text-center py-4">등록된 경기 없음</p>}
                </div>
              </div>
            </div>
          )}
          ...
          {tab === 'record' && (
            <div className="space-y-3">
              {/* Top selector bar */}
              <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl px-3 py-2 flex flex-wrap items-center gap-3">
                <select
                  className="bg-[var(--mm-panel-alt)] border border-[var(--mm-rule)] rounded-lg px-3 py-1.5 text-sm text-[var(--mm-ink)]"
                ...
                <div className="ml-auto flex items-center gap-2 flex-wrap">
                  {lastEventLabel && (
                    <>
                      <span className="text-xs text-[var(--mm-muted)] bg-[var(--mm-panel-alt)] px-2 py-1 rounded">{lastEventLabel}</span>
                  ...
                  {selectedGame?.is_complete && (
                    <button onClick={reopenGame} className="flex items-center gap-1 px-3 py-1.5 bg-[var(--mm-panel-alt)] hover:bg-[var(--mm-rule)] text-[var(--mm-ink)] text-xs rounded-lg transition-colors">
                      기록 재개
                    </button>
                  )}
                </div>
              ...
              {!selectedGame ? (
                <div className="text-center py-20 text-[var(--mm-muted)]">
                  ...
                  <p className="text-xs text-[var(--mm-muted)]">팀 관리 탭에서 먼저 경기를 등록하세요</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
                  ...
                  <div className="lg:col-span-2 space-y-3">
                    {/* Player selector */}
                    <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl p-3">
                      <p className="text-xs text-[var(--mm-muted)] mb-2 font-medium">선수 선택</p>
                      {players.length === 0 && (
                        <p className="text-[var(--mm-muted)] text-xs mb-2">팀 관리 탭에서 선수를 먼저 등록하세요</p>
                      )}
                          ...
                          <button key={p.id} onClick={() => setSelPlayer(p.id)}
                            className={cn(
                              ...
                              selPlayer === p.id
                                ...
                                : 'bg-[var(--mm-panel-alt)] border-[var(--mm-rule)] text-[var(--mm-ink)] hover:border-[color:var(--mm-yellow)]/60 hover:text-[var(--mm-ink)]'
                            )}>
                    ...
                    <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl p-3">
                      {!selPlayer ? (
                        <p className="text-[var(--mm-muted)] text-sm text-center py-4">선수를 먼저 선택하세요</p>
                      ) : pendingShot ? (
                        /* Shot result */
                        <div className="flex flex-col items-center gap-3 py-2">
                          <div className="text-sm text-[var(--mm-ink)]">
                            <span className="font-bold text-[var(--mm-ink)]">
                          ...
                          <button onClick={() => setPendingShot(null)} className="text-xs text-[var(--mm-muted)] hover:text-[var(--mm-ink)]">취소</button>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          <div>
                            <p className="text-xs text-[var(--mm-muted)] mb-2">슈팅</p>
                          ...
                          <div>
                            <p className="text-xs text-[var(--mm-muted)] mb-2">기타</p>
                            <div className="grid grid-cols-3 gap-1.5">
                    ...
                    {events.length > 0 && (
                      <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl p-3">
                        <p className="text-xs text-[var(--mm-muted)] mb-2 font-medium">기록 목록 ({events.length}) — 휴지통으로 개별 삭제</p>
                        <div className="space-y-0.5 max-h-52 overflow-y-auto">
                          {[...events].reverse().map(ev => (
                            <div key={ev.id} className="flex items-center gap-2 text-xs text-[var(--mm-muted)] px-1 py-0.5 hover:bg-[var(--mm-panel-alt)]/50 rounded group">
                              {ev.video_timestamp > 0 && (
                                <span className="text-[var(--mm-muted)] text-xs w-10 shrink-0">{formatTimestamp(ev.video_timestamp)}</span>
                              )}
                              <span className="font-bold text-[var(--mm-ink)] shrink-0">
                              ...
                              {ev.points > 0 && <span className="text-[var(--mm-yellow-strong)]">+{ev.points}</span>}
                              <button
                                onClick={() => deleteEvent(ev.id)}
                                disabled={deletingEventId === ev.id}
                                aria-label="기록 삭제"
                                className="inline-flex items-center justify-center p-1.5 -m-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 text-[var(--mm-muted)] hover:text-red-400 transition-all shrink-0"
                              >
                                <Trash2 size={11} />
                              </button>
                            </div>
                    ...
                    {selectedGame.youtube_url ? (
                      ...
                      <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl flex items-center justify-center aspect-video">
                        <div className="text-center">
                          <p className="text-[var(--mm-muted)] text-sm mb-1">YouTube 영상 없음</p>
                          <p className="text-[var(--mm-muted)] text-xs">팀 관리 탭의 경기 수정에서 YouTube URL을 추가하세요</p>
                        </div>
                      </div>
                    )}

                    {/* Game info + 누적 스코어 */}
                    <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl px-4 py-3 text-sm">
                      <div className="flex items-center gap-4 flex-wrap">
                        <span className="text-[var(--mm-muted)]">{selectedGame.date}</span>
                        {selectedGame.vs_team && <span className="text-[var(--mm-ink)] font-medium">vs {selectedGame.vs_team}</span>}
                        {selectedGame.round && <span className="text-[var(--mm-muted)] text-xs bg-[var(--mm-panel-alt)] px-2 py-0.5 rounded">{selectedGame.round}</span>}
                        {selectedGame.tournament_name && <span className="text-[var(--mm-muted)] text-xs">{selectedGame.tournament_name}</span>}
                        <span className="ml-auto text-[var(--mm-muted)] text-xs">{events.length}개 기록</span>
                      </div>
                      {/* 누적 득점 합계 */}
                      {events.length > 0 && (() => {
                        const total = events.reduce((s, e) => s + e.points, 0)
                        return (
                          <div className="mt-2 pt-2 border-t border-[var(--mm-rule)] flex items-center gap-2">
                            <span className="text-xs text-[var(--mm-muted)]">누적 득점</span>
                            <span className="text-xl font-black font-mono text-[var(--mm-yellow-strong)] ml-auto">{total}</span>
                            <span className="text-xs text-[var(--mm-muted)]">점</span>
                          </div>
                        )
                      })()}
                    </div>
          ...
          {tab === 'analyze' && (
            <div className="space-y-6">
              {!stats && <div className="text-center py-12 text-[var(--mm-muted)]">로딩 중...</div>}
              {stats && stats.total_games === 0 && (
                <div className="text-center py-12 text-[var(--mm-muted)]">기록된 경기가 없습니다. 경기 기록 탭에서 먼저 기록하세요.</div>
              )}

              {stats && stats.total_games > 0 && (
                <>
                  {/* ① 팀 공격 스타일 */}
                  <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl p-5">
                    <h2 className="font-semibold text-[var(--mm-ink)] mb-1">팀 공격 스타일</h2>
                    <p className="text-xs text-[var(--mm-muted)] mb-4">{stats.total_games}경기 누적</p>
                    <TeamShotSummary breakdown={stats.team_shot_breakdown} totalGames={stats.total_games} />

                    {/* Top scorers */}
                    <div className="mt-5 pt-4 border-t border-[var(--mm-rule)]/50">
                      <p className="text-xs text-[var(--mm-muted)] mb-4 font-medium">주요 슈터 (슈팅 시도 상위 7)</p>
                      {(() => {
                        ...
                        const PlayerCard = (p: typeof top7[0], i: number) => {
                          ...
                          const top2 = [...SHOT_TYPES]
                            ...
                            .slice(0, 2)
                          return (
                            <div key={p.player_id} className="bg-[var(--mm-panel-alt)]/40 rounded-xl p-2 sm:p-3 flex flex-col items-center gap-1.5 border border-[var(--mm-rule)]/30">
                              {/* rank + name */}
                              <div className="flex items-center gap-1 w-full">
                                <span className="text-[var(--mm-muted)] text-xs shrink-0">{i + 1}</span>
                                <span className="font-bold text-[var(--mm-ink)] text-xs truncate">
                              ...
                              <div className="flex items-center justify-between w-full" style={{ fontSize: '10px' }}>
                                <span className="text-[var(--mm-muted)]">{p.fgm}/{p.fga}</span>
                                <span className="text-[var(--mm-yellow-strong)] font-bold">{p.pts}pts</span>
                              </div>
                            ...
                            {rest.length > 0 && (
                              <>
                                <div className="flex items-center gap-2">
                                  <div className="flex-1 h-px bg-[var(--mm-rule)]/50" />
                                  <span className="text-xs text-[var(--mm-muted)]">6~7위</span>
                                  <div className="flex-1 h-px bg-[var(--mm-rule)]/50" />
                                </div>
                                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  ...
                  {(() => {
                    ...
                    const SortTh = ({ k, label, cls = 'text-center' }: { k: string; label: React.ReactNode; cls?: string }) => (
                      <th
                        className={`px-3 py-2 font-medium cursor-pointer select-none hover:text-[var(--mm-ink)] transition-colors ${cls}`}
                        onClick={() => toggleSort(k)}
                      >
                        <span className="inline-flex items-center gap-0.5">
                          {label}
                          {sortKey === k ? (
                            <span className="text-[var(--mm-yellow-strong)]">{sortDir === 'desc' ? ' ↓' : ' ↑'}</span>
                          ) : (
                    ...
                    return (
                  <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-[var(--mm-rule)]/50">
                      <h2 className="font-semibold text-[var(--mm-ink)]">선수별 누적 스탯</h2>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-xs text-[var(--mm-muted)] border-b border-[var(--mm-rule)]/50">
                                ...
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  {SHOT_TYPES.map(t => (
                                    <div key={t.type} className="flex items-center gap-0.5">
                                      ...
                                      <span className="text-[var(--mm-muted)] text-xs">{t.label}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                        ...
                        <tbody>
                          {sorted.map(p => (
                            <tr key={p.player_id} className="border-b border-[var(--mm-rule)]/60 hover:bg-[var(--mm-panel-alt)]/30 transition-colors">
                              <td className="px-4 py-2.5">
                                <span className="font-bold text-[var(--mm-ink)]">#{p.player_number}</span>
                                {p.player_name && <span className="ml-1.5 text-[var(--mm-ink)] text-xs">{p.player_name}</span>}
                              </td>
                              <td className="px-3 py-2.5 text-center text-[var(--mm-ink)]">{p.games}</td>
                              <td className="px-3 py-2.5 text-center font-bold text-[var(--mm-yellow-strong)]">{p.pts}</td>
                              <td className="px-3 py-2.5 text-center text-[var(--mm-ink)]">{p.fgm}/{p.fga}</td>
                              <td className={cn('px-3 py-2.5 text-center font-medium', p.fg_pct !== null && p.fg_pct < 30 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                {p.fg_pct !== null ? `${p.fg_pct}%` : '-'}
                              </td>
                              <td className="px-3 py-2.5 text-center text-[var(--mm-ink)]">{p.fg3m}/{p.fg3a}</td>
                              <td className={cn('px-3 py-2.5 text-center font-medium', p.fg3_pct !== null && p.fg3_pct < 30 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                {p.fg3_pct !== null ? `${p.fg3_pct}%` : '-'}
                              </td>
                              <td className="px-3 py-2.5 text-center text-[var(--mm-ink)]">{p.ftm}/{p.fta}</td>
                              <td className={cn('px-3 py-2.5 text-center font-medium', p.ft_pct !== null && p.ft_pct < 60 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                {p.ft_pct !== null ? `${p.ft_pct}%` : '-'}
                              </td>
                              <td className="px-3 py-2.5 text-center text-[var(--mm-ink)]">{p.oreb}</td>
                  ...
                  <div className="bg-[var(--mm-panel)] border border-[var(--mm-rule)]/50 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-[var(--mm-rule)]/50">
                      <h2 className="font-semibold text-[var(--mm-ink)]">경기별 상세</h2>
                    </div>
                    <div className="divide-y divide-[var(--mm-rule)]/60">
                      {stats.games.map(g => (
                        <div key={g.game_id}>
                          <button
                            onClick={() => setExpandedGame(expandedGame === g.game_id ? null : g.game_id)}
                            className="w-full flex items-center gap-3 px-5 py-3 hover:bg-[var(--mm-panel-alt)]/40 transition-colors text-left"
                          >
                            {expandedGame === g.game_id ? <ChevronDown size={14} className="text-[var(--mm-muted)]" /> : <ChevronRight size={14} className="text-[var(--mm-muted)]" />}
                            <span className="text-[var(--mm-ink)] text-sm font-medium">{g.date}</span>
                            {g.vs_team && <span className="text-[var(--mm-ink)] text-xs font-semibold">vs {g.vs_team}</span>}
                            {g.round && <span className="text-[var(--mm-muted)] text-xs bg-[var(--mm-panel-alt)] px-2 py-0.5 rounded">{g.round}</span>}
                            {g.tournament_name && <span className="text-[var(--mm-ink)] text-xs">{g.tournament_name}</span>}
                            <span className="ml-auto text-xs text-[var(--mm-muted)]">{g.players.length}명 기록</span>
                          </button>
                          {expandedGame === g.game_id && (
                            <div className="px-5 pb-4 bg-[var(--mm-ground)]/30">
                              {g.players.length === 0 ? (
                                <p className="text-[var(--mm-muted)] text-xs py-2">이 경기에 기록된 데이터 없음</p>
                              ) : (
                                <div className="overflow-x-auto mt-2">
                                <table className="w-full text-xs">
                                  <thead>
                                    <tr className="text-[var(--mm-muted)] border-b border-[var(--mm-rule)]/50">
                                          ...
                                          <div className="flex items-center gap-1 flex-wrap">
                                            {SHOT_TYPES.map(t => (
                                              <div key={t.type} className="flex items-center gap-0.5">
                                                ...
                                                <span className="text-[var(--mm-muted)]" style={{ fontSize: '9px' }}>{t.label}</span>
                                              </div>
                                            ))}
                                          </div>
                                        </div>
                                  ...
                                  <tbody>
                                    {g.players.map(p => {
                                      ...
                                      return (
                                      <tr key={p.player_id} className="border-b border-[var(--mm-rule)]/40 hover:bg-[var(--mm-panel-alt)]/20">
                                        <td className="py-1.5 pr-3">
                                          <span className="font-bold text-[var(--mm-ink)]">#{p.player_number}</span>
                                          {p.player_name && <span className="ml-1 text-[var(--mm-muted)]">{p.player_name}</span>}
                                        </td>
                                        <td className="py-1.5 text-center font-bold text-[var(--mm-yellow-strong)] px-2">{p.pts}</td>
                                        <td className="py-1.5 text-center text-[var(--mm-ink)] px-2">{p.fgm}/{p.fga}</td>
                                        <td className={cn('py-1.5 text-center font-medium px-2', fgPct !== null && fgPct < 30 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                          {fgPct !== null ? `${fgPct}%` : '-'}
                                        </td>
                                        <td className="py-1.5 text-center text-[var(--mm-ink)] px-2">{p.fg3m}/{p.fg3a}</td>
                                        <td className={cn('py-1.5 text-center font-medium px-2', fg3Pct !== null && fg3Pct < 30 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                          {fg3Pct !== null ? `${fg3Pct}%` : '-'}
                                        </td>
                                        <td className="py-1.5 text-center text-[var(--mm-ink)] px-2">{p.ftm}/{p.fta}</td>
                                        <td className={cn('py-1.5 text-center font-medium px-2', ftPct !== null && ftPct < 60 ? 'text-red-400' : 'text-[var(--mm-ink)]')}>
                                          {ftPct !== null ? `${ftPct}%` : '-'}
                                        </td>
                                        <td className="py-1.5 text-center text-[var(--mm-ink)] px-2">{p.oreb}</td>
