...
import { loadNextDate, resolveQuarterForDate, resolveAssignments, todayYmd, type UpcomingDate } from '@/lib/rsvp/nextGame'
...
export async function GET(
  ...
  let me: { status: Status | null; teamName: string | null; waiting: boolean } | null = null
  if (session) {
    const mine = all.find(r => r.account_id === session.uid)
    const quarter = await resolveQuarterForDate(sb, leagueId, target.date)
    const [assignment] = await resolveAssignments(sb, leagueId, quarter?.id ?? null, [
    ...
    me = {
      ...
      waiting: assignment.waiting && mine?.status === 'going',
    }
  }

  return NextResponse.json({ date: target, me, summary })
}
