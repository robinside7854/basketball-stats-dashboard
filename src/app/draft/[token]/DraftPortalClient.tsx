...
export default function DraftPortalClient({
  ...
  return (
    ...
    >
      ...
      {/* isMyTurn: 다음 차례가 나면 「지금 내 차례」 배지만 띄운다. 시계는 이 연출이 닫힌 뒤
          start-clock 으로 시작하므로 연출을 줄일 이유가 없다(예전 1.2초 축약은 2026-09-18 에 제거). */}
      {/* 1라운드는 이름을 감춘 채 시즌 기록 8칸 → 사진 → 이름 순으로 여는 드라마틱 공개(약 17초).
          탭하면 즉시 공개·닫기, 이름이 다 보인 뒤 5초에 자동 닫힘. */}
      <DraftPickReveal
        data={pickReveal}
