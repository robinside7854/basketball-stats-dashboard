  ...
  return (
    <div className="max-w-lg space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--mm-ink)]">새 Org 추가</h1>
          <p className="text-[var(--mm-muted)] text-sm">새로운 농구 클럽을 등록합니다</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-[var(--mm-panel)] border border-[var(--mm-rule)] rounded-xl p-6 space-y-5">
        <div>
          ...
          <Input value={form.name} onChange={e => handleChange('name', e.target.value)} placeholder="파란날개" className="bg-[var(--mm-panel-alt)] border-[var(--mm-rule)] text-[var(--mm-ink)]" />
        </div>
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Input value={form.org_slug} onChange={e => handleChange('org_slug', e.target.value)} placeholder="paranalgae" className="bg-[var(--mm-panel-alt)] border-[var(--mm-rule)] text-[var(--mm-ink)]" />
          </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-[var(--mm-muted)] mt-1">해당 팀이 경기 기록 편집 시 사용하는 PIN</p>
        </div>
        <div>
        ...
        <div className="pt-2 flex gap-3">
          <Button type="submit" disabled={loading} className="bg-[var(--mm-ink)] text-[var(--mm-panel)] hover:opacity-90 cursor-pointer">
            {loading ? <><Loader2 size={14} className="mr-1.5 animate-spin" />생성 중...</> : 'Org 생성'}
          </Button>
          <Link href="/admin/orgs">
            <Button type="button" variant="outline" className="border-[var(--mm-rule)] text-[var(--mm-ink-soft)] cursor-pointer">취소</Button>
          </Link>
        </div>
      </form>
    </div>
