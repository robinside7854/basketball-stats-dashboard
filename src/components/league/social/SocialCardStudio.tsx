'use client'
// 인스타 카드 생성 스튜디오 — 8장 미리보기 + PNG 저장(html-to-image).
// 주간(from/to)은 URL 로 서버 재조회, 표지 Vol/헤드라인은 로컬 상태(발표용 텍스트).
import { useState, useRef } from 'react'
import { useRouter, usePathname, useSearchParams } from 'next/navigation'
import { toast } from 'sonner'
import type { WeeklyMagazineData } from '@/lib/social/weeklyData'
...
export default function SocialCardStudio({ data }: { data: WeeklyMagazineData }) {
  ...
  const cards = [
    ...
    { id: 'pts', label: '3. 득점 TOP3', node: <LeaderCard data={data} title="득점" metric="POINTS · 이번주" unit="점" leaders={data.ptsTop} /> },
    { id: 'reb', label: '4. 리바운드 TOP3', node: <LeaderCard data={data} title="리바운드" metric="REBOUNDS · 이번주" unit="개" leaders={data.rebTop} /> },
    { id: 'ast', label: '5. 어시스트 TOP3', node: <LeaderCard data={data} title="어시스트" metric="ASSISTS · 이번주" unit="개" leaders={data.astTop} /> },
    { id: 'def', label: '6. 수비 TOP3', node: <LeaderCard data={data} title="수비" metric="STEALS + BLOCKS" unit="" leaders={data.defTop} /> },
    { id: 'best', label: '7. 이번 주 BEST', node: <BestCard data={data} /> },
  ...
  async function savePng(id: string, idx: number) {
    ...
    a.href = dataUrl
    a.download = `miracle-weekly-vol${vol || '1'}-${idx + 1}-${id}.png`
    a.click()
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[var(--mm-muted)] text-sm mt-1">주간 데이터를 4:5(1080×1350) 매거진 카드로. 각 카드 하단 <b>PNG 저장</b> 또는 <b>전체 저장</b>.</p>
      </div>

      {/* 컨트롤 */}
      <div className="flex flex-wrap items-end gap-3 p-4 rounded-md border border-[var(--mm-rule)] bg-[var(--mm-panel)]">
        <label className="flex flex-col gap-1 text-xs font-bold text-[var(--mm-muted)]">주 시작
          <input type="date" value={from} onChange={e => setFrom(e.target.value)} className={inp} />
        </label>
        <label className="flex flex-col gap-1 text-xs font-bold text-[var(--mm-muted)]">주 끝
          <input type="date" value={to} onChange={e => setTo(e.target.value)} className={inp} />
        </label>
        <button onClick={applyWeek} className="px-4 py-2 rounded-md text-sm font-black uppercase tracking-wider bg-[var(--mm-ink)] text-[var(--mm-panel)] cursor-pointer min-h-[40px]">기간 적용</button>
        ...
        <label className="flex flex-col gap-1 text-xs font-bold text-[var(--mm-muted)] flex-1 min-w-[220px]">표지 헤드라인
      ...
      <div className="flex flex-wrap gap-5">
        {cards.map((c, i) => (
          <div key={c.id} className="flex flex-col gap-2">
            ...
            {/* 미리보기: 스케일은 부모에, 캡처 노드(ref)는 원본 1080×1350 유지 */}
      ...
      <p className="text-xs text-[var(--mm-muted)] leading-relaxed">
        · 규격 4:5(1080×1350) · 다크+옐로 고정 테마(앱 라이트/다크와 무관) · 사진은 Supabase 공개 URL(CORS 허용 시 export 반영).
        7번(이번 주 BEST)은 시즌하이/클러치 자리의 v1 — 이후 템플릿 수정요청으로 다듬을 수 있습니다.
      </p>
    </div>
  )
}
