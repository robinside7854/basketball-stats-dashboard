...
import { Play, Square, RotateCcw, Dice5, CheckCircle2, Circle, Crown, Users, RefreshCw } from 'lucide-react'
...
export default function DraftSessionControl({ leagueId, quarterId, teams, authHeaders = {} }: Props) {
  ...
  // silent=true 면 로딩 표시 없이 갱신 (수동 새로고침/액션 후)
  ...
  async function openReady() {
    ...
    setActing(false)
    if (res.ok) { toast.success('준비 체크 시작'); fetchData(true) }
  ...
  async function runLottery(force: boolean) {
    ...
    if (res.ok) { toast.success('추첨 완료 — 드래프트 시작!'); fetchData(true) }
  ...
  async function completeSession() {
    ...
    setActing(false)
    if (res.ok) { toast.success('종료'); fetchData(true) }
    else { const d = await res.json(); toast.error(d.error ?? '실패') }
  }
  ...
  async function resetSession() {
    ...
    setActing(false)
    if (res.ok) { toast.success('리셋 완료'); fetchData(true) }
    else { const d = await res.json(); toast.error(d.error ?? '실패') }
  }
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
      ...
      {draft.status === 'ready_check' && (
        <div>
          ...
          <p className="text-[10px] text-gray-600 mt-1.5">자동 갱신을 끄고 수동 새로고침으로 바꿨습니다. 단장·감독관이 준비를 누른 뒤 새로고침으로 확인하세요.</p>
        </div>
      )}
