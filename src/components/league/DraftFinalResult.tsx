    ...
    <div className="fixed inset-0 z-[60] flex flex-col bg-[#000000]/85 backdrop-blur-sm p-3 sm:p-4"
      ...
      <div className="relative max-w-6xl w-full mx-auto flex flex-col min-h-0 flex-1">
        ...
        >
            ...
            {orderedTeams.map((t, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {list.length === 0 ? (
                    <p className="text-sm text-[color:var(--mm-muted)] text-center py-4">{leaderName ? '추가 픽 없음' : '픽 없음'}</p>
                  ) : (
                    <div className="space-y-1">
                      {list.map(p => (
                        <div key={p.pick_number} className="flex items-center gap-1.5 min-w-0">
                          {!hideOrder && (
                            <span className="text-sm font-mono tabular-nums w-8 shrink-0 text-[color:var(--mm-muted)]">#{p.pick_number}</span>
                          )}
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm sm:text-base text-[#f3f4f6] truncate break-keep min-w-0">
                      ...
                      <span className="text-[color:var(--mm-muted)] mx-1.5">·</span>
                      ...
                      <span className="text-[color:var(--mm-muted)] ml-1.5">({teamName})</span>
                    </span>
                  </span>
                )
              })}
          ...
          {/* 푸터 */}
          <div className="text-center text-xs sm:text-sm text-[color:var(--mm-muted)] pt-2">
