...
await check(
  ...
  rows => {
    ...
    // 2026-08-08: 271 → 273. 2026-05-09 자 빈 슬롯 2건(slot 1·2, 팀 미배정, 이벤트 0건)이
    //   추가돼 있었다. 같은 날짜에 이미 같은 형태의 빈 슬롯(slot 9)이 있었고 미완료 51건 중
    //   26건이 팀 미배정이라, 빈 슬롯 자체는 이 리그의 정상 상태다 — 대관이 안 잡힌 토요일이나
    //   영상이 없어 기록을 못 한 날에 생긴다.
    //   생성 시각 2026-08-08 05:36 — 같은 세션에서 30분 뒤 그날 경기 기록이 시작된 것으로 보아
    //   운영진이 당일 일정을 준비하며 '자동 일정 등록' 을 눌러 옛 날짜의 빠진 슬롯까지
    //   채운 것으로 보인다(2026-05-09 는 slot 9 만 있었고 1·2 가 비어 있었다).
